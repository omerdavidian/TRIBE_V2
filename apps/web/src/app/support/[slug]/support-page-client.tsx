...
function CareItemCard({
  ...
  return (
    <article className={`${cfg.bg} border ${cfg.border} rounded-xl p-5 flex flex-col gap-4`}>
      {/* Header */}
      <div className="flex items-start gap-3">
        <div
          className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${cfg.text} bg-white/60`}
          aria-hidden
        >
          {item.isFulfilled ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.description && (
            <p className="text-xs text-[#40484a] mt-0.5 leading-relaxed line-clamp-2">{item.description}</p>
          )}
          {item.providerProfile?.businessName && (
            <p className="text-[11px] text-[#70797a] mt-1">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <span className={`text-xs font-semibold tabular-nums ${cfg.text}`}>
            ...
            <span className="font-normal text-[#70797a]"> of {money(item.targetAmountCents)}</span>
          </span>
      ...
      {item.isFulfilled ? (
        ...
        <button
          ...
          className={`w-full text-xs font-semibold py-2.5 rounded-lg border-2 transition-all hover:opacity-90 active:scale-[0.98] ${cfg.text} border-current bg-white/50`}
        >
...
function RegistryBlock({
  ...
  return (
    <section className="bg-white rounded-2xl border border-[#e8e2de] overflow-hidden shadow-sm">
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
          <div>
            <h3 className="font-display font-bold text-xl text-[#00343a]">{registry.title}</h3>
            {registry.description && (
              <p className="text-sm text-[#5a6468] mt-1 leading-relaxed max-w-xl">{registry.description}</p>
            )}
            {due && (
              <p className="flex items-center gap-1.5 text-xs text-[#8a9da0] mt-2">
          ...
          {totalTarget > 0 && (
            <div className="flex-shrink-0 text-right min-w-[140px]">
              <p className="text-2xl font-display font-bold text-[#00343a]">{money(totalFunded)}</p>
              <p className="text-xs text-[#8a9da0]">of {money(totalTarget)} goal</p>
              <div className="mt-2 h-1.5 w-full bg-[#e8e2de] rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${barColor} transition-all duration-700`}
                  style={{ width: `${Math.max(percent, percent > 0 ? 2 : 0)}%` }}
                />
              </div>
              <p className="text-xs font-semibold text-[#5a6468] mt-1">{percent}% funded</p>
            </div>
          )}
        </div>
        ...
        {registry.items.length === 0 && (
          <div className="text-center py-8 text-sm text-[#8a9da0]">
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="relative w-full">
        ...
        <div className="bg-gradient-to-b from-white via-white to-white h-24" />
      </div>
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-20 relative z-20 pb-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex items-start gap-4 bg-white rounded-2xl border border-[#e8e1db] p-6">
              <div className="w-16 h-16 rounded-full bg-[#e4f0ee] border-2 border-[#c0d8d5] flex items-center justify-center flex-shrink-0 overflow-hidden">
                {page.user.avatarUrl ? (
                  <Image
                  ...
                  <span className="font-display font-bold text-lg text-[#00343a]">{initials(name)}</span>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-[#70797a]">By</p>
                <p className="font-display font-bold text-xl text-[#00343a]">{name}</p>
                {page.bio && (
                  <p className="text-xs text-[#8a9da0] mt-2 line-clamp-2">{page.bio}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <h2 className="font-display font-bold text-2xl text-[#00343a]">The Impact of Your Support</h2>
                <p className="text-sm text-[#70797a] mt-1">Browse care registries {name} has created</p>
              </div>

              {page.registries.length === 0 ? (
                <div className="text-center py-16 text-[#8a9da0] bg-white rounded-2xl border border-[#e8e1db]">
                  <p className="font-display font-bold text-4xl text-[#e0e8e7] mb-3 select-none">✦</p>
          ...
          <aside className="lg:sticky lg:top-24 space-y-5 h-fit">
            {/* Funding card */}
            <div className="bg-white rounded-2xl border border-[#e8e1db] p-6 space-y-5">
              {grandTotal > 0 ? (
                <>
                  <div>
                    <p className="text-4xl font-display font-bold text-[#00343a]">
                      {money(grandFunded)}
                    </p>
                    <p className="text-xs text-[#70797a] mt-1">
                      raised of {money(grandTotal)} goal
                    </p>
                  </div>
                  <div className="space-y-2">
                    <div className="w-full h-2.5 bg-[#e8e1db] rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-[#29676f] to-[#7d3527] transition-all duration-1000"
                        style={{ width: `${Math.max(grandPct, 1)}%` }}
                      />
                    </div>
                    <p className="text-xs font-semibold text-[#00343a]">{grandPct}% funded • {page.registries.reduce((sum, r) => sum + r.items.length, 0)} items</p>
                  </div>
                ...
                <div className="text-center py-8">
                  <p className="text-sm text-[#8a9da0]">No items added yet</p>
                </div>
              )}
            </div>
            ...
            {page.registries.length > 0 && (
              <div className="bg-white rounded-2xl border border-[#e8e1db] p-6 space-y-4">
                <h3 className="font-semibold text-[#00343a] text-sm">Share This Page</h3>
                <button
                  type="button"
                  onClick={() => setShareOpen(true)}
                  className="w-full border-2 border-[#00343a] hover:bg-[#f9f7f5] text-[#00343a] rounded-xl px-4 py-2.5 font-semibold transition-colors text-sm"
                >
